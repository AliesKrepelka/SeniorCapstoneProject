...
#include <vector>
#include <memory>
#define GLM_ENABLE_EXPERIMENTAL //Aies added
...
GLuint g_array_objects[kNumVaos];
GLuint g_buffer_objects[kNumVaos][kNumVbos];
...
MouseButtonCallback(GLFWwindow* window, int button, int action, int mods)
{
	...
	// FIX: On first press, sync prev values to current cursor position
	// so there's no sudden jump on first click
	if (action == GLFW_PRESS) {
		...
		if (button == GLFW_MOUSE_BUTTON_LEFT) {
			...
			zoom = mx; prev_zoom = mx;
		} else if (button == GLFW_MOUSE_BUTTON_MIDDLE) {
			x_pan = mx; y_pan = my;
			prev_x_pan = mx; prev_y_pan = my;
		}
	}
}
...
int main(int argc, char* argv[])
{
	...
	CHECK_GL_ERROR(glBufferData(GL_ARRAY_BUFFER,
				sizeof(float) * floor_vertices.size() * 4, &floor_vertices[0], GL_STATIC_DRAW));
	CHECK_GL_ERROR(glVertexAttribPointer(0, 4, GL_FLOAT, GL_FALSE, 0, 0));
	CHECK_GL_ERROR(glEnableVertexAttribArray(0));

	CHECK_GL_ERROR(glBindBuffer(GL_ARRAY_BUFFER, g_buffer_objects[kFloorVao][kNormalBuffer]));
	CHECK_GL_ERROR(glBufferData(GL_ARRAY_BUFFER,
				sizeof(float) * floor_normals.size() * 4, &floor_normals[0], GL_STATIC_DRAW));
	CHECK_GL_ERROR(glVertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, 0, 0));
	CHECK_GL_ERROR(glEnableVertexAttribArray(1));
...
if (currentTime - lastTime >= 1.0) {
    ...
    lastTime = currentTime;
		...
		if (g_menger && g_menger->is_dirty()) {
			g_menger->generate_geometry(obj_vertices, vtx_normals, obj_faces);
			g_menger->set_clean();
			std::cout << "Vertex count: " << obj_vertices.size() << "\n"; //testing
			std::cout << "Triangle count: " << obj_faces.size() << "\n";
			CHECK_GL_ERROR(glBindBuffer(GL_ARRAY_BUFFER, g_buffer_objects[kGeometryVao][kVertexBuffer]));
		...
		// Upload geometry vertices every frame
		CHECK_GL_ERROR(glBindBuffer(GL_ARRAY_BUFFER, g_buffer_objects[kGeometryVao][kVertexBuffer]));
		CHECK_GL_ERROR(glBufferData(GL_ARRAY_BUFFER,
		...
		CHECK_GL_ERROR(glUniform1i(mode_location, mode));
